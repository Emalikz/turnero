...
async function submit() {
  ...
  try {
    ...
    successMessage.value = 'Contrasena actualizada. Redirigiendo al dashboard...'

    setTimeout(async () => {
      await router.push({ name: 'tenant-dashboard', params: { slug } })
    }, 1500)
  } catch (error: any) {
    errorMessage.value = error.response?.data?.error?.message ?? 'No se pudo actualizar la contrasena.'
  } finally {
    loading.value = false
  }
}
...
<template>
  <section class="admin-auth-layout">
    <Card class="auth-card">
      <template #title>Cambiar contrasena</template>
      <template #subtitle>Debes cambiar tu contrasena antes de continuar.</template>
      <template #content>
        <form class="stack-md" @submit.prevent="submit">
          ...
          <div class="field-stack">
            <label for="new_password_confirmation">Confirmar nueva contrasena</label>
          ...
          <Button type="submit" label="Actualizar contrasena" icon="pi pi-check" :loading="loading" fluid />
        </form>
      </template>
    </Card>
  </section>
