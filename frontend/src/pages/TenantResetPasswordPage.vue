...
async function submit() {
  ...
  try {
    ...
    errorMessage.value = error.response?.data?.error?.message ?? 'No se pudo restablecer la contraseña.'
  } finally {
    loading.value = false
  }
}
...
<template>
  <section class="admin-auth-layout">
    <Card class="auth-card">
      ...
      <template #subtitle>Crea una nueva contraseña para tu cuenta.</template>
      <template #content>
        <form class="stack-md" @submit.prevent="submit">
          ...
          <Button type="submit" label="Restablecer contraseña" icon="pi pi-check" :loading="loading" fluid />
        </form>
      </template>
    </Card>
  </section>
